...
var scroller = {
	...
	init: function(){
		...
		jQuery(anchor).append(spn);
		jQuery("body").prepend(anchor);
		jQuery(anchor).prepend("<a name='top'></a>");
		jQuery(scrollTopButton).css({
			position: "absolute",
			top: "120px",
			right: "15px",
			color: "#fff",
			...
			padding:"20px 15px",
			borderRadius: "50%",
			boxShadow: "1x 1px 9px 2px #A3A5A6",
			textAlign: "center",
			boxSizing: "content-box"

		});

		jQuery(scrollTopButton).on("mouseover", function(){
			jQuery(this).css({
				backgroundColor: hoverCol
				// opacity: 1
			});
		});
		jQuery(scrollTopButton).on("mouseout", function(){
			jQuery(this).css({
				color: arrowCol,
				backgroundColor: bgCol,
				opacity: op
	...
	offset: function(){
		...
		var arrow = jQuery(".scroll-top-button");
		...
		jQuery(function() {
			...
			jQuery(window).scroll(function() {
				...
				var scrollPosition = eTop - jQuery(window).scrollTop();
